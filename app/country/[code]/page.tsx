import { fetchMoviesByCountry, fetchSeriesByCountry } from "@/lib/tmdb"
...
async function getCountryData(code: string, page: number) {
  const [movies, series] = await Promise.all([
    fetchMoviesByCountry(code, page),
    fetchSeriesByCountry(code, page)
  ])
...
export default async function CountryDetailPage({ params, searchParams }: CountryPageProps) {
  ...
  const { movies, series } = await getCountryData(code, currentPage)
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          <Link href="/country" className="text-zinc-400 hover:text-white text-sm mb-4 inline-block">
            ← Kembali ke Negara
          </Link>
          <h1 className="text-3xl font-bold">Negara: {code.toUpperCase()}</h1>
        ...
        {allContent.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-zinc-400">Belum ada film/series dari negara ini.</p>
          </div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-10">
                {currentPage > 1 && (
                  <Link
                    href={`/country/${code}?page=${currentPage - 1}`}
